...
# "l" is the number of groups being compared
l = nrow(group_stats)

#Call our functions with the statistics we calculated as arguments
btwn_grp_var = b_g_v(group_stats, overall_stats, l)
with_grp_var = w_g_v(group_stats, overall_stats, l, sample_size)
...
Now that we have met the validity conditions, I will demonstrate two methods of finding a p-value using the theory-based approach.

```{r}
#I'll discuss these more later
df_1 = l - 1
df_2 = sample_size - l
...
You may recall that you've used the F-statistic twice already in this class: in linear regression and in evaluating whether variances were approximately equal. This should give you an idea of how valuable ANOVA is in numerous situations. Let's dive in a little bit more on these values in our summary output to better understand what they tell us.

```{r}
summary(anova_model)
```
**Treatment vs. Residuals:** You will see that everything (except the final p-value) is divided between the explanatory variable (the treatment) and the error (the residuals).

**Degrees of Freedom (df):** Hopefully a bit of pattern matching demonstrates that our degrees of freedom calculations are:  
$~~~$ The number of groups minus one ($l - 1$) for our treatment (*Condition*) and  
$~~~$ The number of observations minus the number of groups for the error (or residuals).
