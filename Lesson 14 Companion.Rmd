...
If we are going to put any trust into the p-value of our theory-based approach we better verify the validity conditions. For this test we need to have at least 10 successes and 10 failures in each group.
...
You can also use the built-in function **prop.test()**.  If you choose to use this function, however, it is vital that you "turn off" the continuity correction by including the parameter *correct = FALSE* inside the function. You will get the **incorrect** answer if you do not do this. Also realize that **prop.test()** does not utilize *z* as the standardized statistic but instead utilizes $\chi^{2}$ (chi-squared) which we will learn about later in the course.

```{r}
prop.test(x = c(1975, 255), 
          n = c(1975 + 1627, 255 + 310), 
          alternative = "two.sided",
          correct = FALSE)
```
As you can see from this example, **prop.test** expects a vector of the count of "successes" (number of boys this case) for each group in the *x* parameter and a total number of samples in the *n* parameter. I know it's atypical of me to hard-code numbers but I'm doing it here for sake of clarity.
...
This is a little different than the other interpretation we have learned (we are 95% confident that the true difference in proportions is between 5.3% and 14.1%): however, it provides much more clarity in what we have calculated.
