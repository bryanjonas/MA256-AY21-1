...
```{r}
shifting_t <- function(x, mean, df) {dt(x - mean, df)}
...
replications_dataframe %>%
  ...
  stat_function(fun = shifting_t, 
                args = list(mean = null_mean, df = sample_size - 1),
                color = "red")
```
The code above is here to help demonstrate why a t-distribution is appropriate for modeling our null distribution given the validity conditions are met. The only new part is my custom **shifting_t()** function that I included to move the curve over to our null hypothesis value. As we standardize our sample statistic (to create our standardized statistic) we normally don't have to worry about this shifting and it's just here for a visual. You can see from the plot that the t-distribution isn't a perfect representation but fits our null distribution relatively well. Now that (hopefully) I've got you convinced, let's look at using the **pt** function to calculate our p-value.
